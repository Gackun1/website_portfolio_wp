...
if ( ! function_exists( 'custom_breadcrumb' ) ) {
  function custom_breadcrumb() {
    ...
    if ( is_attachment() ) {
      ...
      echo '<li class="list__item"><span>'. esc_html( $post_title ) .'</span></li>';
    } elseif ( is_single() ) {
      /**
      ...
      if ( $post_type !== 'post' ) {
        ...
        // 投稿自身の表示
        echo '<li class="list__item"><span>'. esc_html( strip_tags( $post_title ) ) .'</span></li>';
      ...
      // 投稿自身の表示
      echo '<li class="list__item"><span>'. esc_html( strip_tags( $page_title ) ) .'</span></li>';
    } elseif ( is_post_type_archive() ) {
      /**
       * 投稿タイプアーカイブページ ( $wp_obj : WP_Post_Type )
       */
      echo '<li class="list__item"><span>'. esc_html( $wp_obj->label ) .'</span></li>';
    } elseif ( is_date() ) {
      /**
       * 日付アーカイブ ( $wp_obj : null )
       */
      ...
      if ( $day !== 0 ) {
        ...
        //年別アーカイブ
        echo '<li class="list__item"><span>'. esc_html( $year ) .'年</span></li>';
      }
    } elseif ( is_author() ) {
      /**
       * 投稿者アーカイブ ( $wp_obj : WP_User )
       */
      echo '<li class="list__item"><span>'. esc_html( $wp_obj->display_name ) .' の執筆記事</span></li>';
    } elseif ( is_archive() ) {
      /**
       ...
       * 検索結果ページ
       */
      echo '<li class="list__item"><span>「'. esc_html( get_search_query() ) .'」で検索した結果</span></li>';
    
    } elseif ( is_404() ) {
      /**
       * 404ページ
       */
      echo '<li class="list__item"><span>お探しの記事は見つかりませんでした。</span></li>';
    } else {
      /**
       * その他のページ（無いと思うけど一応）
       */
      echo '<li class="list__item"><span>'. esc_html( get_the_title() ) .'</span></li>';
    }
